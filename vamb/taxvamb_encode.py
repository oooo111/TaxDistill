...
import torch.nn.functional as F
from peft import LoraConfig, get_peft_model # 用于GenomeOcean的LoRA微调
...
def make_dataloader_concat_hloss(
    ...
    (
        depthstensor, tnftensor, total_abundance_tensor, weightstensor, batchsize, n_workers, cuda,
    ...
    if teacher_embeddings is not None:
        class EmbDataset(_torch.utils.data.Dataset):
            def __init__(self, d, t, ab, w, l, e):
                self.tensors = (d, t, ab, w, l)
                self.embs = e
            def __getitem__(self, index):
                return tuple(tensor[index] for tensor in self.tensors) + (self.embs[index],)
            def __len__(self):
                return len(self.embs)
        dataset = EmbDataset(depthstensor, tnftensor, total_abundance_tensor, weightstensor, labelstensor, teacher_embeddings)
    else:
        dataset = _TensorDataset(depthstensor, tnftensor, total_abundance_tensor, weightstensor, labelstensor)
    ...
    shuffle_loader = True
    if local_rank != -1:
        sampler = _torch.utils.data.DistributedSampler(dataset, shuffle=True)
        shuffle_loader = False

    dataloader = _DataLoader(
        dataset=dataset, batch_size=batchsize, drop_last=len(depthstensor) > batchsize,
        shuffle=shuffle_loader, sampler=sampler, num_workers=n_workers, pin_memory=cuda,
...
class VAMB2Label(_nn.Module):
    ...
    def trainepoch(
        self, data_loader, epoch: int, optimizer, batchsteps: list[int],
        teacher_model=None, tokenizer=None, kd_alpha=0.5, kd_temp=2.0, accumulation_steps: int = 8,
        forward_model=None, forward_teacher=None
    ):
        self.train()
        ...
        epoch_celoss = 0.0; epoch_kdloss = 0.0; epoch_total_loss = 0.0; epoch_correct_labels = 0
        # ========================================================

        if epoch in batchsteps:
            new_batchsize = data_loader.batch_size * 2
            data_loader = _DataLoader(
                dataset=data_loader.dataset, batch_size=new_batchsize, shuffle=False,
                sampler=getattr(data_loader, 'sampler', None), drop_last=len(data_loader.dataset) > new_batchsize,
                num_workers=data_loader.num_workers, pin_memory=data_loader.pin_memory,
        ...
        for i, batch in enumerate(pbar):
            if len(batch) == 6:
                depths_in, tnf_in, abundances_in, weights, labels_in, seqs_in = batch
            else:
                depths_in, tnf_in, abundances_in, weights, labels_in = batch
                seqs_in = None

            if self.usecuda:
                dev = local_rank if is_ddp else 'cuda'
                depths_in, tnf_in, abundances_in = depths_in.to(dev), tnf_in.to(dev), abundances_in.to(dev)
                weights, labels_in = weights.to(dev), labels_in.to(dev)
            ...
            if is_ddp and not is_last_step:
                ...
                sync_context_t = contextlib.nullcontext()

            with sync_context, sync_context_t:
                labels_out = forward_model(depths_in, tnf_in, abundances_in, weights)
                loss_sup_student, correct_labels = self.calc_loss(labels_in, labels_out)
                ...
                if forward_teacher is not None and seqs_in is not None:
                    ...
                    loss_total_step = loss_sup_student

                epoch_total_loss += loss_total_step.item()
                epoch_celoss += loss_sup_student.item()
                epoch_correct_labels += correct_labels.data.item()

                loss = loss_total_step / accumulation_steps
                loss.backward()

            if is_last_step:
                if teacher_model is not None:
                    _torch.nn.utils.clip_grad_norm_(list(self.parameters()) + list(teacher_model.parameters()), max_norm=1.0)
                else:
                    _torch.nn.utils.clip_grad_norm_(self.parameters(), max_norm=1.0)
                optimizer.step()
                optimizer.zero_grad()

            if is_main:
                if forward_teacher is not None and seqs_in is not None:
                    pbar.set_postfix({'Tot_Loss': f"{loss_total_step.item():.4f}", 'Sup_CE': f"{loss_sup_student.item():.4f}", 'KD': f"{loss_kd.item():.4f}"})
                else:
                    pbar.set_postfix({'Loss': f"{loss_total_step.item():.4f}"})

        if is_main:
            avg_total_loss = epoch_total_loss / len(data_loader)
            avg_sup_ce = epoch_celoss / len(data_loader)
            avg_kd_loss = epoch_kdloss / len(data_loader)
            if teacher_model is not None:
                logger.info(f"\tEpoch {epoch + 1} End | Avg Total Loss: {avg_total_loss:.5f} | Avg Sup_CE: {avg_sup_ce:.5f} | Avg KD_Loss: {avg_kd_loss:.5f} | Batchsize: {data_loader.batch_size}")
            else:
                logger.info(f"\tEpoch {epoch + 1} End | Avg Total Loss: {avg_total_loss:.5f} | Avg CE: {avg_sup_ce:.5f} | Batchsize: {data_loader.batch_size}")
                
        self.eval()
        return data_loader

    def trainmodel(
        self, dataloader: _DataLoader[tuple[Tensor, ...]], nepochs: int = 500, lrate: float = 1e-3,
        batchsteps: Optional[list[int]] = [25, 75, 150, 300], modelfile=None,
        ...
        if is_ddp:
            ...
            forward_model = self
            forward_teacher = teacher_model
        # ==============================

        if is_main:
            logger.info("\tNetwork properties ... (Logs only on Master)")

        for epoch in range(nepochs):
            dataloader = self.trainepoch(
                dataloader, epoch, optimizer, sorted(set(batchsteps)),
                teacher_model=teacher_model, tokenizer=tokenizer, kd_alpha=kd_alpha, kd_temp=kd_temp,
            ...
            if is_main and (epoch + 1) % 10 == 0:
                ...
                if teacher_model is not None:
                    ...
                    logger.info(f"\tEpoch {epoch + 1} | Student LR: {current_lr_student:.6e} | Teacher LR: {current_lr_teacher:.6e}")
                else:
                    logger.info(f"\tEpoch {epoch + 1} | Current LR: {current_lr_student:.6e}")

        if is_main and modelfile is not None:
            try:
                self.save(modelfile)
            except:
                pass
